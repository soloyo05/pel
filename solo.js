...
//   #############   COLORES DEL CONTROL REMOTO Y TECLA 0
// Control remoto - teclas de color y tecla 0
window.addEventListener('keydown', (event) => {
  ...
  // Nueva funcionalidad para la tecla "0"
  if (key === '0' || keyCode === 48) { 
      // Prevenir la acción por defecto (por ejemplo, si estuviera en un input)
      event.preventDefault(); 
      // Redirigir a la página del botón de inicio
      window.location.href = document.querySelector('.home-button').href;
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!isNaN(lastIndex) && allItems[lastIndex]) {
    const inPage = (lastIndex >= (currentPage - 1) * itemsPerPage && lastIndex < currentPage * itemsPerPage);
    setTimeout(() => {
      if (inPage) {
  ...
  allItems.forEach((item, i) => {
    item.addEventListener("focus", () => localStorage.setItem("lastFocusIndex", i));
  });
